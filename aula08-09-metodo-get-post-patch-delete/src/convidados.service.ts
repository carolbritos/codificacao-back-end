...
export class ConvidadosService{
    ...
    encontarConvidado(id: number){
        const convidado = this.convidados.find((buscarConvidados) => buscarConvidados.id === id);
        if(!convidado){
    ...
    atualizarIdade(id: number, idade: number){
        const convidado = this.encontarConvidado(id);
    ...
    removerConvidadoLista(id: number){
        ...
        if( index === -1) {
            throw new NotFoundException(`[ADMINISTRADOR] Convidados com ID ${id} não encontrado`);
        }
